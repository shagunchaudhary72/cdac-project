...
const AddPreference = () => {
  ...
  const initCourse = (collegeName) => {
    CollegeService.getCoursesOfCollge(collegeName)
      .then((response) => {
        console.log(college);
  ...
  const getPreferenceList = (id) => {
    StudentService.getPreferences(id)
      .then((res) => {
  ...
  let onAddPreferenceSubmit = (event) => {
    ...
    if (validation()) {
      ...
      StudentService.addPreference(userId, preference)
        .then((response) => {
          console.log(response.data);
          setShow("show");
          getPreferenceList(userId);
  ...
  let handleDelete = (p_id) => {
    StudentService.deletePreference(userId, p_id)
      .then((response) => {
        setShow2("show");
        getPreferenceList(userId);
        setTimeout(function () {
      ...
      <div className="container-fluid w-50 mt-5 add-prefernece-details">
        <div className="m-3">
          ...
          <p className="text-50 text-success mb-3 dashboard-data-section-para">
            Remember!! you should add college according to your priority
          </p>
          <div className="border border-1 rounded">
            <div className="m-3">
              <form onSubmit={onAddPreferenceSubmit} className="row g-1">
                ...
                <button type="submit" className="btn1 primary1 btn-sm mt-3">
                  Add Preference
                </button>
              </form>
            </div>
          ...
          <div className={show} id="snackbar">
            Preference Added Successfully<output></output>
          </div>
          <div className={show2} id="snackbar">
            Preference Deleted Successfully<output></output>
          </div>
        </div>
                ...
                return (
                  <tr key={k + 1}>
                    ...
                    <td>
                      <center>
                        <button
                          className="btn1 btn-sm primary1 rounded"
                          onClick={() => {
                            handleDelete(ele.id);
                          }}
                        >
